...
public class InfantFrame extends JFrame
{
                ...
                public void actionPerformed(ActionEvent e)
                {
                    ...
                    if (returnVal == JFileChooser.APPROVE_OPTION)
                    {
                        ...
                        try
                        {
                            ...
                            // Do the loading work: the first two characters are
                            // the Infant ID
    ...
    private class SelectionPanel extends JPanel
    {
         ...
         * Constructor
         * 
         * Creates a 1x2 grid of components with labels down the left column and
        ...
        private SelectionPanel()
        {
            ...
            // List model contains the data to be displayed. Uses
            //////////////////////////// Trial.toString() to obtain strings
    ...
    public class DataPanel extends JPanel
    {
        ...
        private DataPanel() throws FontFormatException, IOException
        {
            ...
            this.runButton.addActionListener(new ActionListener()
            {
                ...
                public void actionPerformed(ActionEvent e)
                {
                    // Use the button text to decide whether the timer is
                    // running or not.
                    // Based on this information, properly configure the button
                    // text and
                    // the timer.
         ...
         * 
         * Notes: - Deal with the case that 'trial' is null - If the new time is
         * within allowable range, then use that as the current time, set the
         * timeSlider to this time, extract the state for the current time and
         * force an update of the display - If the new time is outside of the
         * allowable range, then ensure that the timer is off
         * 
         * 
         * @param newTime
         *            The new time step
        ...
        /**
         * Update the data display - Each of the kinematic panels - Each of the
         * text fields - Force all components to redraw
         * 
         * @param state
         *            The current state
         */
    ...
    /**
     * Translate the selections made in the SelectionPanel into the
     * configuration of the dataPanel
     * 
     * This method is declared as "synchronized" to ensure that only one thread
     * is allowed to call it and other synchronized methods at once.
     * 
    ...
    public synchronized void update()
    {
        ...
        if (infant != null && infant.getSize() > 0)
        {
            ...
            if (trialIndex >= 0 && trialIndex < infant.getSize())
            {
                ...
                // Reset the time index to the beginning (this will cause an
                // update)
